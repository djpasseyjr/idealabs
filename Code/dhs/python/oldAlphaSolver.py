#!usr/bin/python
#alphaSolver.py
"""This file contains functions I used to learn about the all pass filter 
   when trying to create a stable discrete time perturbation. 
"""
...
from svPerturb import allPassAlpha1
from svPerturb import minAlpha

def evalAP(theta,a):
    n = (1-np.exp(complex(0,-1*theta))*a.conjugate())
    d = (np.exp(complex(0,-1*theta))-a)
    return n/d

def allPassAlpha3(theta,phi):
    return np.exp(complex(0,-1*theta)) - np.exp(complex(0,-.5*(theta+phi)))

def minAllPassAlpha(theta,phi):
    Alpha = []
    NormAlpha = np.array([complex(0,1)]*8)
    Phi = [phi,phi+np.pi,phi+2*np.pi,phi-np.pi]

    for i in range(4):
        Alpha.append(allPassAlpha3(theta,Phi[i]))
        Alpha.append(allPassAlpha2(theta,Phi[i]))
    Alpha = np.array(Alpha)
    
    for i in range(8):
        NormAlpha[i] = la.norm(Alpha[i])
    ind = np.argsort(NormAlpha)[0]
    alpha = Alpha[ind]
    return (alpha,ind)
...
def allPassFMatrix(theta,phi,r):
    ...
    b = np.array([(np.cos(theta+phi)/r)-(np.sin(theta+phi)/r) - 1, 
                  (np.sin(theta+phi)/r) + (np.cos(theta+phi)/r)])
    try:
        ans = la.solve(A,b)
    except Exception:
        ans = np.array([100,0])
...
def graphAPAlphaSol(X=None,Y=None,Z=None,res=100,tol=1e-4,f="norm"):
    ...
    if X is None:
        ...
        for i in range(m):
            for j in range(n):
                alpha = minAlpha(X[i,j],Y[i,j])
                if f == "norm":
                    Z[i,j] = la.norm(alpha)
                if f == "unitary":
                    Z[i,j] = la.norm(alpha) < 1
                if f == "real":
                    Z[i,j] = alpha.real
                if f == "imag":
                    Z[i,j] = alpha.imag
...
def compareSolverSizes():
    ...
    if X is None:
        ...
        for i in range(m):
            for j in range(n):
                if f == "norm":
                    Z[i,j] = la.norm(allPassAlpha3(X[i,j],Y[i,j]))
                if f == "unitary":
                    Z[i,j] = la.norm(allPassAlpha3(X[i,j],Y[i,j])) < 1
                if f == "real":
                    Z[i,j] = allPassAlpha3(X[i,j],Y[i,j]).real
                if f == "imag":
                    Z[i,j] = allPassAlpha3(X[i,j],Y[i,j]).imag
